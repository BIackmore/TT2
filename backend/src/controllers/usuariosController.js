  ...
  try {
    const { rows } = await query(
      `SELECT u.id_usuario, u.nombre, u.correo, u.activo, u.telefono,
              u.fecha_registro, u.perfil, r.nombre AS rol
       FROM usuarios u
...
const toggleEstado = async (req, res, next) => {
  try {
    ...
    const { rows: before } = await query('SELECT activo, correo, nombre FROM usuarios WHERE id_usuario=$1', [id]);
    if (!before.length) return res.status(404).json({ ok: false, error: 'Usuario no encontrado' });

    const nuevo = !before[0].activo;
    await query('UPDATE usuarios SET activo=$1 WHERE id_usuario=$2', [nuevo, id]);

    await audit({
      tabla: 'usuarios', operacion: 'UPDATE', registroId: id,
...
const deleteUser = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { rows } = await query('SELECT correo, nombre FROM usuarios WHERE id_usuario=$1', [id]);
    if (!rows.length) return res.status(404).json({ ok: false, error: 'Usuario no encontrado' });

    await query('DELETE FROM usuarios WHERE id_usuario=$1', [id]);

    await audit({
      tabla: 'usuarios', operacion: 'DELETE', registroId: id,
      cambiadoPor: req.user.correo,
...
const updatePerfil = async (req, res, next) => {
  try {
    ...
    if (req.user.id_usuario !== parseInt(id) && req.user.rol !== 'admin') {
      return res.status(403).json({ ok: false, error: 'Sin permisos para editar este perfil' });
    }
...
const getStats = async (req, res, next) => {
  try {
    const { rows } = await query(
      `SELECT
         COUNT(*) FILTER (WHERE r.nombre='gov')             AS total_gov,
         COUNT(*) FILTER (WHERE r.nombre='user')            AS total_users,
...
module.exports = { getAll, getOne, toggleEstado, deleteUser, updatePerfil, getStats };
